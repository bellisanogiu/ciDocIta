...
\section*{Il form}
Utilizzando un editor di testo, si crei il file \fil{myform.php} nel percorso \sys{/applications/views/}. Esso conterrà il codice del nostro form.
...
Utilizzando un editor di testo, si definisca il file \fil{formsuccess.php} nel percorso \sys{/applications/views/} che informerà l'utente del corretto invio (e quindi validazione) del form.
...
Utilizzando un editor di testo, si crei il file \fil{form.php} nel percorso \sys{/applications/controllers/}.
...
Se al momento si invia il modulo, il form viene caricato nuovamente: questo avviene perché non si è ancora stabilita alcuna regola di convalida. Infatti, poiché la classe Form Validation non sa come comportarsi, questa restituisce FALSE per impostazione predefinita. La funzione \var{run()} restituisce TRUE solo se le regole sono state rispettate senza alcuna eccezione.
...
Si ricarichi il form e si utilizzi come nome utente la parola ``test'': si può vedere che il campo data del form viene passato alla funzione di callback per essere elaborata.
...
Se non ci sono errori, ovviamente nulla verrà visualizzato, in caso contrario questo comparirà il valore errato. Un aspetto a cui prestare attenzione è che quando si utilizza un array come nome di un campo del form, è necessario specificarlo anche nella funzione:
...
Le regole di validazione così definite vengono caricate automaticamente e utilizzate quando si chiama la funzione \var{run()}.

Per quanto riguarda invece la loro organizzazione in gruppi, è necessario inserire le regole in sotto array. Si consideri il seguente esempio in cui vengono definiti due gruppi di regole che abbiamo chiamato ``signup'' e ``email'':
...
A questo punto se si desidera richiamare solo un determinato gruppo di regole, è necessario passare il suo nome alla funzione \var{run()}. Per esempio se si volesse chiamare il solo gruppo signup si dovrà procedere nel seguente modo:
...
Una possibilità alquanto interessante è quella di associare un gruppo di regole ad una funzione del proprio Controller in modo da richiamare automaticamente le regole di validazione desiderate. Il metodo si basa sul chiamare il gruppo di regole nello stesso identico modo della coppia ``controller'' e ``metodo'' che si desiderano utilizzare. Per esempio, se si possiede un controller di nome \var{Member} e di un metodo chiamato \var{signup} ecco come apparirà il suo prototipo:
...
Quando un gruppo di regole si chiama come un controller/metodo, esso verrà utilizzato automaticamente quando la funzione \var{run()} sarà invocata dalla classe/metodo.
...
oppure, se si una un array multidimensionale:
