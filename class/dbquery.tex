...
La funzione \var{query()} restituisce un oggetto quando vengono eseguite le query di tipo read, utilizzate per visualizzare i risultati (per esempio \var{SELECT}).
...
Se per qualsiasi ragione è necessario cambiare il prefisso, senza creare una nuova connessione, si può utilizzare la funzione:
